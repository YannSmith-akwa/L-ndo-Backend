...
-- ────────────────────────────────────────────────────────────
-- Vente au guichet (13/09/2026) — reservations.mode_paiement
-- n'autorisait que les paiements en ligne (mtn_momo / orange_money).
-- Une vente au guichet est payée en espèces sur place : on élargit la
-- contrainte plutôt que d'ajouter une colonne séparée, 'especes' est
-- une valeur de mode_paiement comme les autres pour tout le reste du
-- code (reversements, exports, etc.), qui n'a besoin de rien savoir
-- de plus. Sur une base déjà existante, `create table if not exists`
-- plus haut ne retouche pas une contrainte déjà en place : ces deux
-- lignes ALTER sont ce qui la met à jour pour de vrai. Suppose le nom
-- de contrainte par défaut de Postgres pour un check de colonne
-- (`{table}_{colonne}_check`) — si l'ALTER échoue, vérifier le nom
-- réel via `\d reservations` ou pg_constraint.
alter table reservations drop constraint if exists reservations_mode_paiement_check;
alter table reservations add constraint reservations_mode_paiement_check check (mode_paiement in ('mtn_momo', 'orange_money', 'especes'));
