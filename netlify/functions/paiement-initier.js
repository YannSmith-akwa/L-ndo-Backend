...
// Anti-harcèlement, par numéro CIBLE (voir audit du 22/08/2026) —
// s'applique uniquement à MTN, seul opérateur ici à pousser un prompt
// USSD automatiquement vers `tel` (Orange fonctionne par redirection
// ouverte par le payeur lui-même, voir plus bas).
const FENETRE_CIBLE_MINUTES = 15;
const MAX_INITIATIONS_PAR_CIBLE = 5;

// Second plafond, par IP cette fois (voir simulation d'attaque du
// 31/08/2026, point « limite contournable avec plusieurs numéros
// réels ») : la limite par numéro cible ci-dessus n'arrête pas un
// attaquant qui contrôle plusieurs comptes vérifiés (numéros réels
// distincts), chacun ciblant une victime DIFFÉRENTE — chaque victime
// reste sous son propre plafond de 5, mais l'attaquant en touche
// beaucoup. Seuil volontairement large (20 contre 5) : plusieurs vrais
// clients au même comptoir/même Wi-Fi ne doivent pas se bloquer les uns
// les autres sur un chemin qui touche de l'argent réel — l'objectif
// n'est pas d'arrêter un usage légitime groupé, juste de plafonner un
// script qui arrose beaucoup de numéros depuis une seule source.
const MAX_INITIATIONS_PAR_IP = 20;

// Même en-tête que otp-envoyer.js, pour la même raison (voir son
// commentaire) : x-nf-client-connection-ip posée par Netlify lui-même,
// plus fiable qu'un x-forwarded-for qu'un client pourrait falsifier.
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (operateur === 'mtn') {
      // telephone_paiement est saisi librement par le client à cette
      // étape (fonctionnalité voulue : payer pour un tiers) — combiné à
      // l'absence de limite sur le nombre de réservations créées, un
      // compte authentifié pouvait déclencher un prompt USSD MTN vers
      // n'importe quel numéro camerounais, en boucle, sans coût pour
      // l'attaquant. Le cooldown de 30s plus haut ne protège qu'UNE
      // réservation, pas le numéro cible lui-même.
      const [{ count: initiationsCibleCount }] = await sql(
        `select count(*)::int as count from paiement_tentatives
         where telephone = $1 and cree_le > now() - interval '${FENETRE_CIBLE_MINUTES} minutes'`,
        [tel.e164]
      );
      if (initiationsCibleCount >= MAX_INITIATIONS_PAR_CIBLE) {
        return erreur(429, 'Trop de tentatives de paiement vers ce numéro récemment — réessayez dans quelques minutes.');
      }

      const ip = ipClient(event);
      if (ip) {
        const [{ count: initiationsIpCount }] = await sql(
          `select count(*)::int as count from paiement_tentatives
           where ip = $1 and cree_le > now() - interval '${FENETRE_CIBLE_MINUTES} minutes'`,
          [ip]
        );
        if (initiationsIpCount >= MAX_INITIATIONS_PAR_IP) {
          return erreur(429, 'Trop de tentatives de paiement récemment depuis votre connexion — réessayez dans quelques minutes.');
        }
      }
      ...
      await sql('insert into paiement_tentatives (telephone, ip) values ($1, $2)', [tel.e164, ip]);
    } else {
