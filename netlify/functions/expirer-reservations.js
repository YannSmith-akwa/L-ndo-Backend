...
// audit, point 3.5), tentatives_connexion (même logique, ajoutée avec
// les comptes agence) et paiement_tentatives (audit du 22/08/2026, même
// logique encore) — sans purge, ces tables grossiraient sans limite à
// mesure que le volume réel augmente.
...
const RETENTION_PAIEMENT_JOURS = 2;
...
exports.handler = async () => {
  try {
    ...
    const purgePaiement = await sql(
      `delete from paiement_tentatives where cree_le < now() - interval '${RETENTION_PAIEMENT_JOURS} days' returning id`
    );

    console.log(`expirer-reservations : ${expirees[0]?.n ?? 0} réservation(s) expirée(s), ${purgeOtp.length} tentative(s) OTP purgée(s), ${purgeConnexion.length} tentative(s) de connexion purgée(s), ${purgePaiement.length} tentative(s) de paiement purgée(s).`);
