...
// audit, point 3.5 : cette table ne se purgeait jamais).
...
exports.handler = async () => {
  try {
    ...
    const expirees = await sql.query(
      `with a_expirer as (
    ...
    const purge = await sql.query(
      `delete from otp_tentatives where cree_le < now() - interval '${RETENTION_OTP_JOURS} days' returning id`
    );

    console.log(`expirer-reservations : ${expirees[0]?.n ?? 0} réservation(s) expirée(s), ${purge.length} tentative(s) OTP purgée(s).`);
