...
exports.handler = async (event) => {
  ...
  try {
    const [agence] = await sql.query('select id, nom from agences where id = $1', [agence_id]);
    if (!agence) return erreur(404, 'Agence introuvable.');

    const [trajet] = await sql.query('select id from trajets where id = $1 and agence_id = $2', [trajet_id, agence_id]);
    if (!trajet) return erreur(404, 'Trajet introuvable.');

    const [t] = await sql.query('select prix_simple as "prixSimple", prix_ar as "prixAr", commission from tarifs where id = 1');
